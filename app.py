...
<style>
    ...
    html, body, [class*="css"] { font-family: 'Plus Jakarta Sans', sans-serif; background-color: #07090E; color: #E2E8F0; }
    .stApp { background: radial-gradient(circle at 15% 15%, rgba(16, 24, 40, 0.8) 0%, #07090E 100%); }
    section[data-testid="stSidebar"] { background-color: #0B0F17 !important; border-right: 1px solid rgba(255, 255, 255, 0.06); }
    
    .glass-card {
        background: rgba(15, 23, 42, 0.55); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px);
        border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 14px; padding: 22px; margin-bottom: 20px;
        box-shadow: 0 4px 24px -1px rgba(0, 0, 0, 0.5);
    }
    .glass-card-accent { background: linear-gradient(135deg, rgba(16, 185, 129, 0.08) 0%, rgba(15, 23, 42, 0.6) 100%); border: 1px solid rgba(16, 185, 129, 0.25); }

    .ai-justification-box {
        background: linear-gradient(135deg, rgba(59, 130, 246, 0.1) 0%, rgba(15, 23, 42, 0.7) 100%);
        border-left: 4px solid #3B82F6; padding: 18px; border-radius: 0 12px 12px 0; margin-top: 20px;
        font-size: 0.95rem; line-height: 1.6; color: #93C5FD;
    }

    .kpi-title { font-family: 'JetBrains Mono', monospace; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.12em; color: #94A3B8; margin-bottom: 6px; }
    .kpi-value { font-family: 'Plus Jakarta Sans', sans-serif; font-size: 2.1rem; font-weight: 700; color: #F8FAFC; line-height: 1.1; }
    .kpi-badge { display: inline-flex; align-items: center; gap: 6px; font-family: 'JetBrains Mono', monospace; font-size: 0.72rem; padding: 3px 8px; border-radius: 6px; margin-top: 10px; }
    .badge-green { background: rgba(16, 185, 129, 0.15); color: #10B981; border: 1px solid rgba(16, 185, 129, 0.3); }
    .badge-amber { background: rgba(245, 158, 11, 0.15); color: #F59E0B; border: 1px solid rgba(245, 158, 11, 0.3); }
    .badge-red { background: rgba(239, 68, 68, 0.15); color: #EF4444; border: 1px solid rgba(239, 68, 68, 0.3); }

    .pulse-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: #10B981; box-shadow: 0 0 0 0 rgba(16, 185, 129, 0.7); animation: pulse 1.8s infinite; margin-right: 8px; }
    .pulse-dot.red { background: #EF4444; box-shadow: 0 0 0 0 rgba(239, 68, 68, 0.7); animation: pulse-red 1s infinite; }
    ...
    .terminal-console { background-color: #030712; border: 1px solid #1F2937; border-radius: 8px; padding: 16px; font-family: 'JetBrains Mono', monospace; font-size: 0.78rem; color: #10B981; height: 180px; overflow-y: auto; margin-top: 15px; }
...
with st.sidebar:
    ...
    if prompt := st.chat_input("Ask O.M.E.G.A..."):
        ...
        with chat_container.chat_message("assistant"):
            if GEMINI_AVAILABLE:
                sys_prompt = f"You are O.M.E.G.A., the AI core of LEVIATHAN OS. Answer concisely in 2 sentences. User query: {prompt}"
                try:
                    response = gemini_model.generate_content(sys_prompt).text
                except Exception:
                    response = "API error. System nominal, supply chains stable."
            else:
                response = "Processing query via fallback protocol. Operations remain optimal."
...
if screen == "Fleet Operations":
    ...
    with col2:
        ...
        b_html = '<div class="kpi-badge badge-red"><span class="pulse-dot red"></span>Typhoon Detected</div>' if simulate_anomaly else '<div class="kpi-badge badge-amber">Standard Congestion</div>'
        st.markdown(f'<div class="glass-card"><div class="kpi-title">Active Bottlenecks</div><div class="kpi-value" style="color: {"#EF4444" if simulate_anomaly else "#F8FAFC"}">{choke_count}</div>{b_html}</div>', unsafe_allow_html=True)
    with col3:
    ...
    with c_term:
        ...
        if simulate_anomaly:
            logs = f"<div><span class='term-time'>[{t1}]</span> <span class='term-crit'>[CRITICAL]</span> TYPHOON IN BAY OF BENGAL</div><div><span class='term-time'>[{t2}]</span> <span class='term-sys'>[SYS]</span> HALTING AIS LEGACY ROUTES</div><div><span class='term-time'>[{t3}]</span> <span style='color:#10B981;'>[SUCCESS]</span> AUTONOMOUS REROUTE: 42 BYPASSED</div>"
        else:
    ...
    if GEMINI_AVAILABLE:
        try:
            briefing_text = gemini_model.generate_content(f"Write a 2-sentence executive briefing for logistics managers. State: {ai_status}. Tone: Cold, analytical.").text
        except:
    ...
    if GEMINI_AVAILABLE:
        try:
            justification = gemini_model.generate_content("As an AI logistics commander, write a 3-sentence technical justification for why routing a ship on a 140 NM detour to avoid a storm saves 21% fuel compared to idling in a chokepoint.").text
             ...
             justification = "O.M.E.G.A Core Analysis: While the bypass route introduces a 140 Nautical Mile spatial deviation, it eliminates 38.5 hours of static anchorage wait time. By avoiding continuous auxiliary generator burn in the chokepoint queue, overall fuel efficiency increases by 21.3%. This translates to immediate capital retention."
    else:
        justification = "O.M.E.G.A Core Analysis: While the bypass route introduces a 140 Nautical Mile spatial deviation, it eliminates 38.5 hours of static anchorage wait time. By avoiding continuous auxiliary generator burn in the chokepoint queue, overall fuel efficiency increases by 21.3%. This translates to immediate capital retention."
